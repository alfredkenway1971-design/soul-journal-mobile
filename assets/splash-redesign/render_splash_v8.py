...
def make_canvas(W, H, k, off_frac=0.42):
    ...
    for y in range(SH):
        ...
        for x in range(0, SW, 4):
            ...
            peach_mix = max(0.0, tx - 0.45) / 0.55 * max(0.0, 1 - ty / 0.40)
            c = lerp(base, BG_PEACH, peach_mix * 0.55)
...
def render(W, H, out, off_frac=0.42):
    ...
    draw_leaf(d, X, Y, k, cx=MASTER / 2, tip_y=270, base_y=690,
              wmax=104, bend=22, ink=LEAF_INK, fill=LEAF_FILL, fill_alpha=150,
              tilt=math.radians(40), stem_len=70)
    add_text(d, X, Y, k, MASTER / 2, 830, "Soul", SERIF, 150, TITLE, 4,
             variation={"wght": 500})
    add_text(d, X, Y, k, MASTER / 2, 990, "Journal", SERIF, 132, TITLE, 4,
             variation={"wght": 500})
...
render(1600, 1600, "/root/soul-journal-mobile/assets/splash-redesign/splash-1600-v8.png")
render(1242, 2688, "/root/soul-journal-mobile/assets/splash-redesign/splash-portrait-v8.png", off_frac=0.40)
